...
const tableName = 'savings_accounts_leaderboard_history'

class leaderBoardSavingsAccounts {
  constructor () {
  ...
  async process (leaderData) {
    ...
    const vertical = 'Savings Accounts' // this value should match with ultimate leaderboard vertical
    ...
    console.log(`savings-accounts for ${collectionDate}`)
    ...
    try {
      ...
      for (let i=0; leaderboards.length > i; i++) {
        ...
        if (leaderboardRankings.length) {
          let filename = `savings-accounts_${this.collectionDate}_${this.currentLeaderboard.slug}.csv`
  ...
  leaderRank () {
    ...
    this.Ratings.forEach((rating) => {
      let obj = {
        ...
        initialdeposit: rating.initialdeposit,
        monthlydeposit: rating.monthlydeposit,
        monthlyperiod: rating.monthlyperiod,
        totalinterestearned: parseFloat(rating.totalinterestearned),
        withinlimit: rating.withinlimit,
...
module.exports = leaderBoardSavingsAccounts
