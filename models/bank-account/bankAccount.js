...
BankAccount.add({
	...
	hasNPP: { type: Types.Select, options: availableOptions.all, default: availableOptions.unknown },
	hasFreeInternationalAccountTransfer: { type: Types.Select, options: availableOptions.all, default: availableOptions.unknown },
	holdForeignCurrency: { type: Types.Select, options: availableOptions.all, default: availableOptions.unknown },
	numberOfBranches: { type: Types.Number, default: 0 },
...
BankAccount.schema.pre('save', async function (next) {
  ...
  if (utils.slug(this.slug.toLowerCase()) !== this.slug) {
	...
	await discontinuedService(this, { urlPrefix: '/bank-accounts' })
