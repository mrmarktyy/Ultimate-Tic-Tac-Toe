...
async function prepDataAndPushToRedshift (date, personalLoans, personalLoanVariations, qualifications) {
  ...
  const headers = [
    ...
    'isFullTimeEmploymentAccepted', 'isPartTimeEmploymentAccepted', 'isContractEmploymentAccepted',
...
async function insertIntoRedshift (rows, headers, filename, table) {
  if (rows.length > 0) {
    ...
    await awsUploadToS3(`personal-loans-history/${process.env.REDSHIFT_DATABASE}/${filename}`, csv, 'redshift-2node')

    let command = `delete from ${table} where filename = $1`
    await redshiftQuery(command, [filename])
    command = `copy ${table} from 's3://redshift-2node/personal-loans-history/${process.env.REDSHIFT_DATABASE}/${filename}' credentials 'aws_access_key_id=${process.env.S3_KEY};aws_secret_access_key=${process.env.S3_SECRET}' EMPTYASNULL CSV ACCEPTINVCHARS TRUNCATECOLUMNS`
