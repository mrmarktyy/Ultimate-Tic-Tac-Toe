...
module.exports = async function () {
  ...
  try {
    let sqlCommand = 'select s1.uuid, s1.cpc_price, s1.cpa_price, s1.cpaa_price FROM salesforce_products s1 JOIN (SELECT uuid, MAX(date_start) date_start FROM salesforce_products GROUP BY uuid) s2 on s1.uuid = s2.uuid AND s1.date_start = s2.date_start'
